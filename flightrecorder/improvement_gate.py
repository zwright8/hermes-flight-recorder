...
def evaluate_improvement_ledger_gate(
    ...
    *,
    ...
    schema_check = check_schema_contract(ledger, name_or_id="improvement_ledger")
    if schema_check["passed"] is not True:
        details = "; ".join(str(error) for error in schema_check["errors"][:5])
        raise ImprovementLedgerGateError(f"Improvement-ledger gate input failed improvement_ledger schema: {details}")
    from .validation import validate_improvement_ledger_payload_consistency

    semantic_check = validate_improvement_ledger_payload_consistency(ledger)
    if semantic_check.errors:
        details = "; ".join(semantic_check.errors[:5])
        raise ImprovementLedgerGateError(f"Improvement-ledger gate input failed semantic validation: {details}")
...
def _decision_summary(passed: bool, checks: list[dict[str, Any]], metrics: dict[str, Any]) -> dict[str, Any]:
    ...
    next_actions = _decision_next_actions("improvement_ledger_gate", "Improvement-ledger gate", blocking_checks)
    return {
        "readiness": readiness,
