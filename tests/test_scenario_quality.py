...
class ScenarioQualityTests(unittest.TestCase):
    def test_scenario_quality_scores_bundled_suite(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(metrics["task_family_count"], 4)
            self.assertEqual(metrics["average_contract_score"], 89.17)
            self.assertEqual(metrics["min_contract_score"], 65)
            self.assertEqual(metrics["observable_scenario_rate"], 0.8333)
