...
class TrainingGateTests(unittest.TestCase):
    def test_gate_export_accepts_demo_policy(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result["metrics"]["trainer_view_source_fingerprint_coverage"]["rows"], 11)
            self.assertEqual(result["metrics"]["trainer_view_source_fingerprint_coverage"]["fully_verified"], 11)
            ...
            self.assertEqual(result["metrics"]["task_completion"]["check_pass_rate"], 0.5652)
            self.assertEqual(result["metrics"]["trace_signal"]["average_event_count"], 6.0)
            self.assertEqual(result["metrics"]["trace_signal"]["tool_or_api_episode_rate"], 0.8571)
            self.assertEqual(result["metrics"]["trace_signal"]["risk_count"], 2)
            self.assertEqual(result["metrics"]["dataset_splits"]["task_family_count"], 5)
            self.assertEqual(result["metrics"]["dataset_splits"]["train_episode_count"], 4)
            ...
            self.assertEqual(result["policy"]["effective"]["max_task_completion_incomplete"], 4)
            self.assertEqual(result["policy"]["effective"]["min_task_completion_check_pass_rate"], 0.5652)
            self.assertEqual(result["policy"]["effective"]["min_trace_average_events"], 6.0)
            ...
            self.assertEqual(result["policy"]["effective"]["min_split_task_families"], 5)
            self.assertEqual(result["policy"]["effective"]["min_train_episodes"], 4)
    ...
    def test_gate_export_fails_thresholds_and_quality_flags(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result["decision"]["recommendation"], "block_iteration")
            self.assertEqual(result["decision"]["blocking_check_count"], len(result["failed_checks"]))
            self.assertEqual(result["next_action_count"], 1)
    ...
    def test_gate_export_fails_task_completion_thresholds(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            metrics["task_completion"]["incomplete_count"] = 5
